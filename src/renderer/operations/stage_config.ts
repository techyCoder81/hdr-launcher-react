...
import { StageParams, stageInfo } from './stage_info';
...
export class StageConfig {
  ...
  async load(): Promise<void> {
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        backend
          .fileExists(root + ACTIVE_CONFIG_FILE)
          .then(async (exists) => {
            if (!exists) {
  ...
  async getAll(): Promise<StageParams[]> {
    return new Promise<StageParams[]>(async (resolve, reject) => {
      try {
        await this.load();
        let list = this.dom?.querySelectorAll('[hash=db_root]')[0];
        if (list === undefined) {
        ...
        for (var i = 0; i < list.children.length; ++i) {
          ...
          stages.push(new StageParams(name_id, !disp_order.includes('-1')));
        }
        resolve(stages);
      } catch (e) {
        reject(e);
