...
export class StageInfo {
  ...
  async list(): Promise<Stage[]> {
    this.loadCache();
    let values = [...StageInfo.display_map.values()];
    return values;
  }

  private loadCache() {
    if (StageInfo.display_map.size == 0) {
        let ids = Object.keys(stageInfo);
        for (const nameId of ids) {
            let display = stageInfo[nameId]?.display_name;
            StageInfo.display_map.set(display, {name_id: nameId, display_name: display});
            StageInfo.id_map.set(nameId, {name_id: nameId, display_name: display});
        }
    }
  }
}
